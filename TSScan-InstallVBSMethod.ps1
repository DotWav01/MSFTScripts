...
.DESCRIPTION
    Launched via ServiceUI_x64.exe → Launch-TSScan.vbs → wscript.exe to bridge
    the SYSTEM context to the active user session, allowing the TSScan Server
    interactive installer wizard to be displayed to the user. After the wizard
    completes, the site license file is copied to the installation directory.
...
.NOTES
    ...
    Deployment:
        ...
        Install  : ServiceUI_x64.exe -Process:explorer.exe wscript.exe ".\Launch-TSScan.vbs"
